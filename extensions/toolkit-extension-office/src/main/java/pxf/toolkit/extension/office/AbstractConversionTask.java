...
import static pxf.toolkit.extension.office.OfficeEnvironment.SERVICE_DESKTOP;
import static pxf.toolkit.extension.office.OfficeEnvironment.toUnoProperties;
...
public abstract class AbstractConversionTask implements OfficeTask {
  ...
  private XComponent loadDocument(OfficeContext context, File inputFile) throws OfficeException {
    ...
    XComponentLoader loader =
        UnoRuntime.queryInterface(XComponentLoader.class, context.getService(SERVICE_DESKTOP));
    Map<String, ?> loadProperties = getLoadProperties(inputFile);
    XComponent document = null;
    try {
      document =
          loader.loadComponentFromURL(
              Cast.urlFromFile(inputFile), "_blank", 0, toUnoProperties(loadProperties));
    } catch (ErrorCodeIOException errorCodeIOException) {
      throw new OfficeException(
          "could not load document: "
              + inputFile.getName()
              + "; errorCode: "
              + errorCodeIOException.ErrCode,
          errorCodeIOException);
    } catch (IllegalArgumentException | IOException illegalArgumentException) {
      throw new OfficeException(
          "could not load document: " + inputFile.getName(), illegalArgumentException);
    }
    if (document == null) {
  ...
  private void storeDocument(XComponent document, File outputFile) throws OfficeException {
    ...
    try {
      UnoRuntime.queryInterface(XStorable.class, document)
          .storeToURL(Cast.urlFromFile(outputFile), toUnoProperties(storeProperties));
    } catch (ErrorCodeIOException errorCodeIOException) {
      throw new OfficeException(

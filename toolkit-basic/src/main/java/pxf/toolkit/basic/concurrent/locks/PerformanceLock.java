...
import pxf.toolkit.basic.exception.IllegalCallException;
 ...
 *
 * <p>注意：如果逻辑中关闭了该锁（锁失效），则不能有效保护并发数据
 *
 * @author potatoxf
 * @date 2020/11/2
 */
@ThreadSafe
public class PerformanceLock extends ReentrantSpinLock {

  private static final Logger LOG = LoggerFactory.getLogger(PerformanceLock.class);
   ...
   */
  public final void writeLock(boolean op) {
    if (checkLockFailure()) {
      return;
    }
    initLock();
   ...
   * @param op 加锁 {@code true}，解锁 {@code false}
   */
  public final void readLock(boolean op) {
    if (checkLockFailure()) {
      return;
    }
    initLock();
